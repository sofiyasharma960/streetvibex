...
export default function CheckoutPage({ cart, setCart }) {
  ...
  // Ref to prevent logic racing during rapid clicks
  ...
  const processOrder = async (isRazorpay = false, rzpResponse = null) => {
    try {
      const endpoint = isRazorpay ? "/api/orders/verify" : "/api/orders/cod";
      ...
      const body = {
        ...
        userId: user?._id,
        ...(isRazorpay && rzpResponse) // Inject RZP IDs if online payment
      };
  ...
  const handlePayment = async () => {
    // 10/10 UX: Prevent submission if already working, invalid, or cart empty
    ...
    try {
      ...
      const options = {
        ...
        theme: { color: "#00FFFF" },
        handler: (res) => {
          // Pass the RZP response to our verification endpoint
