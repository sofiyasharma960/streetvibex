    ...
    <>
      <nav className={`fixed top-0 w-full z-[100] transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-12 flex justify-between items-center">
          ...
          <Link 
            to="/" 
            className="text-xl font-black tracking-[-0.05em] text-white hover:text-[#00FFFF] transition-all"
            style={{ textShadow: scrolled ? "none" : "0 0 15px rgba(0,255,255,0.3)" }}
          >
            ...
            >
              ...
              {cartCount > 0 && (
                <motion.span 
                  ...
                  className="bg-[#00FFFF] text-black text-[9px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-[0_0_10px_rgba(0,255,255,0.5)]"
                >
                  {cartCount}
                </motion.span>
              )}
          ...
          >
            {["shop", "about", "contact"].map((item) => (
              <button
                key={item}
                onClick={() => handleNav(item)}
                className="text-2xl font-black tracking-[0.3em] uppercase hover:text-[#00FFFF]"
              >
                {item}
              </button>
            ))}
            ...
            {!user ? (
              <Link to="/login" onClick={() => setMenuOpen(false)} className="text-sm tracking-widest text-[#00FFFF]">SIGN IN</Link>
            ) : (
              <button onClick={() => { logout(); setMenuOpen(false); }} className="text-sm tracking-widest text-red-500">SIGN OUT</button>
            )}
          </motion.div>
        )}
      </AnimatePresence>
